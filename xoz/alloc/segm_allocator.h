...
class SegmentAllocator {
    ...
    public:
        explicit SegmentAllocator(
                Repository& repo, uint16_t max_inline_sz = 8, bool coalescing_enabled = true,
...
no_free_space:
    ...
    private:
        uint32_t allocate_extents(
                Segment& segm, uint32_t blk_cnt_remain, uint32_t max_ext_cnt,
                bool ignore_max_ext_cnt, bool use_parent) {
            ...
            while (blk_cnt_remain and frag_level_ok) {
                ...
                if (result.success) {
                    ...
                    frag_level_ok = (allocated + predicted_remaining < max_ext_cnt) \
                                    or ignore_max_ext_cnt;
