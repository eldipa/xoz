...
namespace {
    // The base array's blocks of 64 byte and the segment array of 128 bytes
    // makes a 2 to 1 ratio (growing 1 blocks the segment block array grows in
    // 2 block the base array)
    const uint32_t base_blkarr_blk_sz = 64;
    //const uint32_t base_blkarr_subblk_sz = 4;
    const uint8_t base_blkarr_blk_sz_order = 6;
    const uint32_t blkarr_blk_sz = 128;

    // NOTE: SegmentBlockArrayTest64128 is a parametrized test that will run for each
    // possible flag for SegmentBlockArray that does not change the visible output
    class SegmentBlockArrayTest64128 : public testing::TestWithParam<uint32_t> {
    };

    TEST_P(SegmentBlockArrayTest64128, OneBlock) {
    ...
    TEST_P(SegmentBlockArrayTest64128, OneBlockTwice) {
    ...
    TEST_P(SegmentBlockArrayTest64128, OneBlockCompletely) {
    ...
    TEST_P(SegmentBlockArrayTest64128, TwoBlocks) {
    ...
    TEST_P(SegmentBlockArrayTest64128, MaxBlocks) {
        ...
        // Note: due the size of the block size, this last block has the same value than the first block
        // This is because the data cycles and has nothing to do with how the blk array works.
        // It is just luck.
    ...
    TEST_P(SegmentBlockArrayTest64128, ZeroBlocks) {
    ...
    TEST_P(SegmentBlockArrayTest64128, ExtentOutOfBoundsSoFail) {
    ...
    TEST_P(SegmentBlockArrayTest64128, OneBlockButWriteLessBytes) {
    ...
    TEST_P(SegmentBlockArrayTest64128, OneBlockButWriteAtOffset) {
    ...
    TEST_P(SegmentBlockArrayTest64128, OneBlockBoundary) {
    ...
    TEST_P(SegmentBlockArrayTest64128, ShrinkByDeallocExtents) {
        ...
        XOZ_EXPECT_SIZES(sg,
                2, // 1 extent
                base_blkarr_blk_sz * 2 // allocated space (measured in base array blk size)
                );

        // Because growing 1 blk makes grow the underlying array grow by 2 blks,
        // we expect a new non-suballoc extent in the segment of length 2 blk.
        ...
        EXPECT_EQ(sg.exts().back().is_suballoc(), false);
        EXPECT_EQ(sg.exts().back().blk_cnt(), (uint32_t)4);
        ...
        EXPECT_EQ(sg.ext_cnt(), (uint32_t)2);
        EXPECT_EQ(sg.exts().back().is_suballoc(), false);
        EXPECT_EQ(sg.exts().back().blk_cnt(), (uint32_t)6);
        ...
        // Now we release_blocks which should split the last extent of 2 (segm) blks
        // to release the 2 pending blks leaving 2 extents of 1 blk each
        // In terms of base's blk sz, the last extent has 6 blks and we will
        // release 4.
        sg_blkarr.release_blocks();
        XOZ_EXPECT_SIZES(sg,
                4, // 2 extent
                base_blkarr_blk_sz * 2 * 2
                );
    ...
    TEST_P(SegmentBlockArrayTest64128, SegmentWithInlineWillFail) {
    ...
    INSTANTIATE_TEST_SUITE_P(
            SegmentBlockArrayTest64128MultiFlags,
            SegmentBlockArrayTest64128,
            testing::Values(0, SG_BLKARR_REALLOC_ON_GROW),
            [](const testing::TestParamInfo<SegmentBlockArrayTest64128::ParamType>& info) {
                switch (info.param) {
                case 0:
                    return "ZeroFlags";
