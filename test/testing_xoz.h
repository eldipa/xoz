...
namespace testing_xoz {
    namespace helpers {
        ...
        std::string hexdump(const xoz::IOSegment& io, unsigned at = 0, unsigned len = unsigned(-1));
        bool are_all_zeros(const char* buf, unsigned buf_sz, unsigned at = 0, unsigned len = unsigned(-1));
